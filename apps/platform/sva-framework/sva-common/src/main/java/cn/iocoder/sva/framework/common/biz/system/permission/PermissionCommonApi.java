package cn.iocoder.sva.framework.common.biz.system.permission;
...
public interface PermissionCommonApi {
    ...
    @GetMapping(PREFIX + "/get-user-role-ids")
    @Operation(summary = "获得用户拥有的角色编号集合")
    @Parameter(name = "userId", description = "用户编号", example = "1", required = true)
    CommonResult<Set<Long>> getUserRoleIds(@RequestParam("userId") Long userId);

    @GetMapping(PREFIX + "/get-login-user-all-role-ids")
    @Operation(summary = "获得登录用户的全部角色编号集合(包含直接分配和部门分配的角色)")
    @Parameter(name = "userId", description = "用户编号", example = "1", required = true)
    CommonResult<Set<Long>> getLoginUserAllRoleIds(@RequestParam("userId") Long userId);
