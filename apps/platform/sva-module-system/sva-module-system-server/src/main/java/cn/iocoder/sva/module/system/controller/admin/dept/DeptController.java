...
import cn.iocoder.sva.module.system.controller.admin.dept.vo.dept.DeptRespVO;
import cn.iocoder.sva.module.system.controller.admin.dept.vo.dept.DeptSaveReqVO;
...
public class DeptController {
    ...
    @Operation(summary = "获取子部门列表", description = "用于部门树懒加载")
    public CommonResult<List<DeptSimpleRespVO>> getDeptChildren(@RequestParam(value = "parentId", required = false) Long parentId) {
        if (parentId == null) {
            parentId = DeptDO.PARENT_ID_ROOT;
        }
        List<DeptDO> list = deptService.getDeptChildren(parentId);
        return success(BeanUtils.toBean(list, DeptSimpleRespVO.class));
    }
