package cn.iocoder.sva.module.system.service.permission;
...
import cn.iocoder.sva.module.system.dal.mysql.permission.RoleMapper;
...
import static cn.iocoder.sva.module.system.enums.LogRecordConstants.*;
...
public class RoleServiceImpl implements RoleService {
    ...
    public Long createRole(RoleSaveReqVO createReqVO, Integer type) {
        ...
        // 判断当前是否是超级管理员，若不是，默认和父角色一致
        if(!createReqVO.getFlag()){
            Long creatorRole = createReqVO.getCreatorRole();
            RoleDO roleDO = roleMapper.selectById(creatorRole);
            if(roleDO != null){
                // 2. 插入到数据库
                RoleDO role = BeanUtils.toBean(createReqVO, RoleDO.class)
                        .setCreatorRole(createReqVO.getCreatorRole())
                        ...
                        .setDataScope(roleDO.getDataScope())
                        .setDataScopeDeptIds(roleDO.getDataScopeDeptIds());
    ...
    @Override
    @CacheEvict(value = RedisKeyConstants.ROLE, key = "#id")
    public void updateRoleDeptScope(Long id, Integer dataScope, Set<Long> dataScopeDeptIds) {
        // 校验是否可以更新
//        validateRoleForUpdate(id);
        deptRoleMapper.delete(new  QueryWrapper<DeptRoleDO>().eq("role_id", id));
        List<DeptRoleDO> deptRoleDOS = new ArrayList<>();
        for (Long deptId : dataScopeDeptIds) {
            DeptRoleDO deptRoleDO = new DeptRoleDO();
            deptRoleDO.setDeptId(deptId);
            deptRoleDO.setRoleId(id);
            deptRoleDOS.add(deptRoleDO);
        }
        deptRoleMapper.insertBatch(deptRoleDOS);
    ...
    @Override
    public List<RoleDO> getRoleList(QueryWrapper<RoleDO> wrapper) {
    ...
    @Override
    public List<RoleDO> getRoleListSelf(RolePageReqVO reqVO) {

        return roleMapper.selectListSelf(reqVO);
    }
    @Override
    public List<RoleDO> getRoleSelf(Set<Long> roleList) {

        return roleMapper.selectRoleSelf(roleList,0);
    }
