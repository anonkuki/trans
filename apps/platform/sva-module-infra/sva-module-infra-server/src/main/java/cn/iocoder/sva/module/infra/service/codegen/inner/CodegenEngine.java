package cn.iocoder.sva.module.infra.service.codegen.inner;
...
import cn.iocoder.sva.framework.apilog.core.enums.OperateTypeEnum;
import cn.iocoder.sva.framework.common.exception.util.ServiceExceptionUtil;
...
import cn.iocoder.sva.framework.common.util.date.LocalDateTimeUtils;
...
import cn.iocoder.sva.framework.common.util.string.StrUtils;
...
import cn.iocoder.sva.module.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 sva-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 sva-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.iocoder.sva.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/sva-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/sva-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "sva-module-${table.moduleName}/" + // 顶级模块
                "sva-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "sva-module-${table.moduleName}/" + // 顶级模块
                "sva-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "sva-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "sva-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "sva-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "sva-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
