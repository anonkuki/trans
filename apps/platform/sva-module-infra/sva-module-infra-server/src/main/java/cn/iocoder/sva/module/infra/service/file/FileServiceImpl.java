...
import cn.iocoder.sva.module.infra.controller.admin.file.vo.file.FileCreateReqVO;
import cn.iocoder.sva.module.infra.controller.admin.file.vo.file.FilePageReqVO;
import cn.iocoder.sva.module.infra.controller.admin.file.vo.file.FilePresignedUrlRespVO;
import cn.iocoder.sva.module.infra.dal.dataobject.file.FileDO;
import cn.iocoder.sva.module.infra.dal.mysql.file.FileMapper;
import cn.iocoder.sva.module.infra.framework.file.core.client.FileClient;
import cn.iocoder.sva.module.infra.framework.file.core.utils.FileTypeUtils;
...
import static cn.iocoder.sva.module.infra.enums.ErrorCodeConstants.FILE_NOT_EXISTS;
...
public class FileServiceImpl implements FileService {
    ...
    public void deleteFileByUrl(String url) throws Exception {
        ...
        if (StrUtil.isNotBlank(path)) {
            try {
                client.delete(path);
                log.info("[deleteFileByUrl] 成功删除 MinIO 文件，路径: {}", path);
            } catch (Exception e) {
                log.error("[deleteFileByUrl] 删除 MinIO 文件失败，路径: {}", path, e);
                throw e;
            }
        } else {
            log.warn("[deleteFileByUrl] 提取的路径为空，无法删除文件");
        }
    }
    ...
    private String extractPathFromUrl(String url) {
        try {
            ...
            if (firstSlashIndex != -1) {
                String path = url.substring(firstSlashIndex + 1);
            ...
            log.error("[extractPathFromUrl] 提取路径失败: {}", url, e);
            return null;
        }
    }
