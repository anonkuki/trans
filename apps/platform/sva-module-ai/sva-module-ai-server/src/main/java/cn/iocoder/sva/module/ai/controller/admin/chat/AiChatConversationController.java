...
import cn.iocoder.sva.module.ai.controller.admin.chat.vo.conversation.AiChatConversationPageReqVO;
import cn.iocoder.sva.module.ai.controller.admin.chat.vo.conversation.AiChatConversationRespVO;
...
public class AiChatConversationController {
    ...
    public CommonResult<PageResult<AiChatConversationRespVO>> getChatConversationPage(AiChatConversationPageReqVO pageReqVO) {
        ...
        // 获取所有用户ID，批量查询用户信息
        List<Long> userIds = convertList(pageResult.getList(), AiChatConversationDO::getUserId);
        ...
        return success(BeanUtils.toBean(pageResult, AiChatConversationRespVO.class,
                conversation -> {
                    conversation.setMessageCount(messageCountMap.getOrDefault(conversation.getId(), 0));
                    conversation.setUserName(finalUserNameMap.get(conversation.getUserId()));
                }));
    }
