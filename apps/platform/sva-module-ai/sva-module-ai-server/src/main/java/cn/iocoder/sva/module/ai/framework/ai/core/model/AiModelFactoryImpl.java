...
import cn.iocoder.sva.framework.common.util.spring.SpringUtils;
import cn.iocoder.sva.module.ai.enums.model.AiPlatformEnum;
import cn.iocoder.sva.module.ai.framework.ai.config.AiAutoConfiguration;
import cn.iocoder.sva.module.ai.framework.ai.config.YudaoAiProperties;
import cn.iocoder.sva.module.ai.framework.ai.core.model.baichuan.BaiChuanChatModel;
import cn.iocoder.sva.module.ai.framework.ai.core.model.doubao.DouBaoChatModel;
import cn.iocoder.sva.module.ai.framework.ai.core.model.gemini.GeminiChatModel;
import cn.iocoder.sva.module.ai.framework.ai.core.model.hunyuan.HunYuanChatModel;
import cn.iocoder.sva.module.ai.framework.ai.core.model.midjourney.api.MidjourneyApi;
import cn.iocoder.sva.module.ai.framework.ai.core.model.siliconflow.SiliconFlowApiConstants;
import cn.iocoder.sva.module.ai.framework.ai.core.model.siliconflow.SiliconFlowChatModel;
import cn.iocoder.sva.module.ai.framework.ai.core.model.siliconflow.SiliconFlowImageApi;
import cn.iocoder.sva.module.ai.framework.ai.core.model.siliconflow.SiliconFlowImageModel;
import cn.iocoder.sva.module.ai.framework.ai.core.model.suno.api.SunoApi;
import cn.iocoder.sva.module.ai.framework.ai.core.model.xinghuo.XingHuoChatModel;
...
public class AiModelFactoryImpl implements AiModelFactory {
    ...
    public ChatModel getOrCreateChatModel(AiPlatformEnum platform, String apiKey, String url, String model, Double temperature) {
        String cacheKey = buildClientCacheKey(ChatModel.class, platform, apiKey, url, model, temperature);
        return Singleton.get(cacheKey, (Func0<ChatModel>) () -> {
            // noinspection EnhancedSwitchMigration
            switch (platform) {
                case TONG_YI:
                    return buildTongYiChatModel(apiKey, url, model, temperature);
                case YI_YAN:
                    return buildYiYanChatModel(apiKey, model, temperature);
                case DEEP_SEEK:
                    return buildDeepSeekChatModel(apiKey, model, temperature);
                case DOU_BAO:
                    return buildDouBaoChatModel(apiKey, model, temperature);
                case HUN_YUAN:
                    return buildHunYuanChatModel(apiKey, url, model, temperature);
                case SILICON_FLOW:
                    return buildSiliconFlowChatModel(apiKey, model, temperature);
                case ZHI_PU:
                    return buildZhiPuChatModel(apiKey, url, model, temperature);
                case MINI_MAX:
                    return buildMiniMaxChatModel(apiKey, url, model, temperature);
                case MOONSHOT:
                    return buildMoonshotChatModel(apiKey, url, model, temperature);
                case XING_HUO:
                    return buildXingHuoChatModel(apiKey, model, temperature);
                case BAI_CHUAN:
                    return buildBaiChuanChatModel(apiKey, model, temperature);
                case OPENAI:
                    return buildOpenAiChatModel(apiKey, url, model, temperature);
                case AZURE_OPENAI:
                    return buildAzureOpenAiChatModel(apiKey, url, model, temperature);
                case ANTHROPIC:
                    return buildAnthropicChatModel(apiKey, url, model, temperature);
                case GEMINI:
                    return buildGeminiChatModel(apiKey, model, temperature);
                case OLLAMA:
                    return buildOllamaChatModel(url, model, temperature);
                case GROK:
                    return buildGrokChatModel(apiKey, url, model, temperature);
                default:
    ...
    private static ChatModel buildTongYiChatModel(String key, String url, String model, Double temperature) {
        ...
        if (isMultiModalModel(modelName)) {
            log.info("Using DashScope native SDK for multi-modal model: {}", modelName);
            return new DashScopeNativeChatModel(key, modelName);
        }
        
        // 使用 Spring AI Alibaba 的标准实现
        log.info("Using Spring AI Alibaba for model: {}", modelName);
        DashScopeApi.Builder builder = DashScopeApi.builder().apiKey(key);
    ...
    /**
     * 判断是否是需要多模态支持的模型
     * Qwen3 系列模型包括：qwen3-*, qwen3-vl-* 等
     */
    private static boolean isMultiModalModel(String modelName) {
        if (modelName == null) {
            return false;
        }
        // Qwen3 系列模型或包含 vl（vision-language）的模型需要多模态支持
        return modelName.startsWith("qwen3") || modelName.contains("qwen3") || 
               modelName.contains("vl") || modelName.contains("VL");
    }
    /**
     ...
     */
    private static QianFanChatModel buildYiYanChatModel(String key, String model, Double temperature) {
        ...
        // 使用传入的model和temperature设置默认选项
        QianFanChatOptions.Builder optionsBuilder = QianFanChatOptions.builder();
        ...
        return new QianFanChatModel(qianFanApi, optionsBuilder.build());
    }
     ...
     */
    private static DeepSeekChatModel buildDeepSeekChatModel(String apiKey, String model, Double temperature) {
        DeepSeekApi deepSeekApi = DeepSeekApi.builder().apiKey(apiKey).build();
        // 使用传入的model，如果为空则使用默认值
        String modelName = StrUtil.isNotBlank(model) ? model : DeepSeekApi.DEFAULT_CHAT_MODEL.getName();
        // 使用数据库中的温度参数，如果为空则使用默认值 0.7
        double temp = temperature != null ? temperature : 0.7;
        DeepSeekChatOptions options = DeepSeekChatOptions.builder().model(modelName)
...
//                .build();
     ...
     */
    private ChatModel buildDouBaoChatModel(String apiKey, String model, Double temperature) {
        YudaoAiProperties.DouBao properties = new YudaoAiProperties.DouBao()
                .setApiKey(apiKey);
        // 如果传入了model，设置到properties中
        if (StrUtil.isNotBlank(model)) {
            properties.setModel(model);
        }
        // 如果传入了temperature，设置到properties中
        if (temperature != null) {
     ...
     */
    private ChatModel buildHunYuanChatModel(String apiKey, String url, String model, Double temperature) {
     ...
     */
    private ChatModel buildSiliconFlowChatModel(String apiKey, String model, Double temperature) {
     ...
     */
    private ZhiPuAiChatModel buildZhiPuChatModel(String apiKey, String url, String model, Double temperature) {
        ZhiPuAiApi.Builder zhiPuAiApiBuilder = ZhiPuAiApi.builder().apiKey(apiKey);
        if (StrUtil.isNotEmpty(url)) {
            zhiPuAiApiBuilder.baseUrl(url);
        }
        // 使用传入的model，如果为空则使用默认值
        String modelName = StrUtil.isNotBlank(model) ? model : ZhiPuAiApi.DEFAULT_CHAT_MODEL;
        // 使用数据库中的温度参数，如果为空则使用默认值 0.7
        double temp = temperature != null ? temperature : 0.7;
        ZhiPuAiChatOptions options = ZhiPuAiChatOptions.builder().model(modelName).temperature(temp).build();
     ...
     */
    private MiniMaxChatModel buildMiniMaxChatModel(String apiKey, String url, String model, Double temperature) {
        MiniMaxApi miniMaxApi = StrUtil.isEmpty(url) ? new MiniMaxApi(apiKey)
                : new MiniMaxApi(url, apiKey);
        // 使用传入的model，如果为空则使用默认值
        String modelName = StrUtil.isNotBlank(model) ? model : MiniMaxApi.DEFAULT_CHAT_MODEL;
        // 使用数据库中的温度参数，如果为空则使用默认值 0.7
        double temp = temperature != null ? temperature : 0.7;
        MiniMaxChatOptions options = MiniMaxChatOptions.builder().model(modelName).temperature(temp).build();
     ...
     */
    private MoonshotChatModel buildMoonshotChatModel(String apiKey, String url, String model, Double temperature) {
        ...
        // 使用传入的model，如果为空则使用默认值
        String modelName = StrUtil.isNotBlank(model) ? model : MoonshotApi.DEFAULT_CHAT_MODEL;
        // 使用数据库中的温度参数，如果为空则不设置（使用Moonshot默认值）
        MoonshotChatOptions.Builder optionsBuilder = MoonshotChatOptions.builder().model(modelName);
        if (temperature != null) {
            optionsBuilder.temperature(temperature);
        }
        MoonshotChatOptions options = optionsBuilder.build();
     ...
     */
    private static XingHuoChatModel buildXingHuoChatModel(String key, String model, Double temperature) {
        List<String> keys = StrUtil.split(key, '|');
     ...
     */
    private BaiChuanChatModel buildBaiChuanChatModel(String apiKey, String model, Double temperature) {
     ...
     */
    private static OpenAiChatModel buildOpenAiChatModel(String openAiToken, String url, String model, Double temperature) {
     ...
     */
    private static AzureOpenAiChatModel buildAzureOpenAiChatModel(String apiKey, String url, String model, Double temperature) {
        // TODO @芋艿：使用前，请测试，暂时没密钥！！！
        OpenAIClientBuilder openAIClientBuilder = new OpenAIClientBuilder()
                .endpoint(url).credential(new KeyCredential(apiKey));
        // 注意：Azure OpenAI需要在创建时指定deploymentName（即model）
        // 这里需要根据实际情况调整，可能需要从url或apiKey中提取
     ...
     */
    private static AnthropicChatModel buildAnthropicChatModel(String apiKey, String url, String model, Double temperature) {
     ...
     */
    private static GeminiChatModel buildGeminiChatModel(String apiKey, String model, Double temperature) {
     ...
     */
    private static OllamaChatModel buildOllamaChatModel(String url, String model, Double temperature) {
        OllamaApi ollamaApi = OllamaApi.builder().baseUrl(url).build();
        // 使用数据库中的温度参数，如果为空则不设置（使用Ollama默认值）
    ...
    private ChatModel buildGrokChatModel(String apiKey, String url, String model, Double temperature) {
