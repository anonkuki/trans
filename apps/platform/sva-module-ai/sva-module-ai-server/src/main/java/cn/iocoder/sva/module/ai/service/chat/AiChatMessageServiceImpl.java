package cn.iocoder.sva.module.ai.service.chat;
...
import cn.iocoder.sva.framework.tenant.core.util.TenantUtils;
import cn.iocoder.sva.module.ai.controller.admin.chat.vo.message.AiChatMessagePageReqVO;
import cn.iocoder.sva.module.ai.controller.admin.chat.vo.message.AiChatMessageRespVO;
...
import cn.iocoder.sva.module.ai.dal.dataobject.chat.AiChatMessageDO;
import cn.iocoder.sva.module.ai.dal.dataobject.knowledge.AiKnowledgeDocumentDO;
...
import cn.iocoder.sva.module.ai.dal.dataobject.model.AiToolDO;
import cn.iocoder.sva.module.ai.dal.mysql.chat.AiChatMessageMapper;
import cn.iocoder.sva.module.ai.enums.ErrorCodeConstants;
...
import cn.iocoder.sva.module.ai.framework.ai.core.webserch.AiWebSearchRequest;
import cn.iocoder.sva.module.ai.framework.ai.core.webserch.AiWebSearchResponse;
import cn.iocoder.sva.module.ai.service.knowledge.AiKnowledgeDocumentService;
import cn.iocoder.sva.module.ai.service.knowledge.AiKnowledgeSegmentService;
import cn.iocoder.sva.module.ai.service.knowledge.bo.AiKnowledgeSegmentSearchReqBO;
import cn.iocoder.sva.module.ai.service.knowledge.bo.AiKnowledgeSegmentSearchRespBO;
...
import cn.iocoder.sva.module.ai.service.model.AiToolService;
import cn.iocoder.sva.module.ai.util.AiUtils;
import cn.iocoder.sva.module.ai.util.FileTypeUtils;
...
import static cn.iocoder.sva.module.ai.enums.ErrorCodeConstants.CHAT_CONVERSATION_NOT_EXISTS;
import static cn.iocoder.sva.module.ai.enums.ErrorCodeConstants.CHAT_MESSAGE_NOT_EXIST;
...
public class AiChatMessageServiceImpl implements AiChatMessageService {
    ...
    @Autowired(required = false) // 由于 sva.ai.web-search.enable 配置项，可以关闭 AiWebSearchClient 的功能，所以这里只能不强制注入
    private AiWebSearchClient webSearchClient;

    @SuppressWarnings("SpringJavaAutowiredFieldsWarningInspection")
    @Autowired(required = false) // 由于 sva.ai.mcp.client.enable 配置项，可以关闭 McpSyncClient 的功能，所以这里只能不强制注入
    ...
    private Prompt buildPrompt(AiChatConversationDO conversation, List<AiChatMessageDO> messages,
        ...
        if (toolCallbacks != null && !toolCallbacks.isEmpty()) {
            log.info("[buildPrompt] 已加载 {} 个工具", toolCallbacks.size());
            toolCallbacks.forEach(tc -> 
                log.info("[buildPrompt] 工具名称: {}", tc.getToolDefinition().name()));
        } else {
            log.warn("[buildPrompt] 未加载任何工具, roleId={}", conversation.getRoleId());
        }
    ...
    private List<ToolCallback> getToolCallbackListByRoleId(Long roleId) {
        log.debug("[getToolCallbackListByRoleId] 开始获取工具列表, roleId={}", roleId);
        if (roleId == null) {
            log.debug("[getToolCallbackListByRoleId] roleId为空，返回null");
            return null;
        }
        AiChatRoleDO chatRole = chatRoleService.getChatRole(roleId);
        if (chatRole == null) {
            log.warn("[getToolCallbackListByRoleId] 角色不存在, roleId={}", roleId);
            return null;
        }
        List<ToolCallback> toolCallbacks = new ArrayList<>();
        // 1. 通过 toolIds
        if (CollUtil.isNotEmpty(chatRole.getToolIds())) {
            log.info("[getToolCallbackListByRoleId] 角色配置的工具ID: {}", chatRole.getToolIds());
            Set<String> toolNames = convertSet(toolService.getToolList(chatRole.getToolIds()), AiToolDO::getName);
            log.info("[getToolCallbackListByRoleId] 解析到的工具名称: {}", toolNames);
            toolNames.forEach(toolName -> {
                ToolCallback toolCallback = toolCallbackResolver.resolve(toolName);
                if (toolCallback != null) {
                    toolCallbacks.add(toolCallback);
                    log.debug("[getToolCallbackListByRoleId] 成功解析工具: {}", toolName);
                } else {
                    log.warn("[getToolCallbackListByRoleId] 无法解析工具: {}", toolName);
                }
            });
        } else {
            log.warn("[getToolCallbackListByRoleId] 角色未配置工具ID");
        }
        // 2. 通过 mcpClients
        ...
        log.info("[getToolCallbackListByRoleId] 最终返回 {} 个工具", toolCallbacks.size());
    ...
    private AiChatMessageDO createChatMessage(Long conversationId, Long replyId,
        ...
        AiChatMessageDO message = new AiChatMessageDO().setConversationId(conversationId).setReplyId(replyId)
                ...
                .setType(messageType.getValue()).setContent(StrUtil.nullToDefault(content, "")).setUseContext(useContext)
