<template>
  <Dialog v-model="dialogVisible" title="批量上传">
    <el-upload
      ref="uploadRef"
...
import { useGlossaryItemUpload } from '@/components/UploadFile/src/useGlossaryItemUpload'
import { UploadFile, UploadProgressEvent } from 'element-plus/es/components/upload/src/upload'

defineOptions({ name: 'TranGlossaryItemImportForm' })
...
const data = ref({ parentId: 0 })
const uploadRef = ref()

const { uploadUrl, httpRequest } = useGlossaryItemUpload(() => data.value.parentId)
...
const handleFileChange = (file: UploadFile) => {
  data.value.parentId = data.value.parentId
}
