  ...
  <div class="translator-container">
    ...
    <el-card class="config-card" shadow="hover">
      <div class="config-row">
        ...
        <!-- 选择角色 -->
        <div class="config-item">
          <el-select v-model="selectedRoleId" placeholder="选择角色" style="width: 140px;">
            <el-option
        ...
        <!-- 选择术语库下拉框 -->
        <div class="config-item">
          <el-select v-model="selectedGlossaryId" placeholder="选择术语库" clearable style="width: 140px;">
            <el-option
    ...
    <el-card class="result-card" shadow="hover" style="margin-top: 16px;">
      <!-- 翻译中状态 -->
      <div v-if="taskId" class="task-running">
        <div class="timer">
      ...
      <div v-if="taskStatus === 'completed'" class="task-completed">
        ...
        <div class="filter-row">
          <el-checkbox v-model="showNewOnly">仅显示未在术语库中的条目</el-checkbox>
...
const selectedGlossaryId = ref(null)
const modelList = ref([])
...
const filteredEditTable = computed(() => {
  ...
  if (showNewOnly.value) {
    data = data.filter(item => item.status !== "完全命中")
  }
...
const startTranslate = async () => {
  ...
  try {
    const res = await TranApi.submitTranslate({
      ...
      glossaryId: selectedGlossaryId.value,
...
const downloadFile = async (type) => {
  if (taskId.value) {
    try {
      await TranApi.downloadFile(taskId.value, type)
    } catch (error) {
...
// 直接使用 MinIO URL 下载（像文档列表页面那样）
const downloadDirect = async (type) => {
  if (!minioUrls.value[type]) {
    ElMessage.error('文件下载链接不存在')
    return
  }

  try {
    const url = minioUrls.value[type]
    ...
    if (type === 'file' && selectedFile.value) {
      ...
      filename = targetName + '_双语对照.docx'
    } else if (type === 'excel' && selectedFile.value) {
      const originalName = selectedFile.value.name
      const dotIndex = originalName.lastIndexOf('.')
      const url = minioUrls.value[type]
      const urlPath = new URL(url).pathname
      const ext = urlPath.substring(urlPath.lastIndexOf('.')) || '.xlsx'
      filename = originalName.substring(0, dotIndex) + '_双语对照表' + ext
    } else if (type === 'qc' && selectedFile.value) {
      const originalName = selectedFile.value.name
      const dotIndex = originalName.lastIndexOf('.')
      const url = minioUrls.value[type]
      const urlPath = new URL(url).pathname
      const ext = urlPath.substring(urlPath.lastIndexOf('.')) || '.pdf'
      filename = originalName.substring(0, dotIndex) + '_QC报告' + ext
    } else {
