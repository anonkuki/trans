...
defineOptions({ name: 'SystemTempFileForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      const data = await TempFileApi.getTempFile(id)
      formData.value = data
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as TempFileApi.TempFileVO
    if (formType.value === 'create') {
      await TempFileApi.createTempFile(data)
      message.success(t('common.createSuccess'))
    } else {
      await TempFileApi.updateTempFile(data)
