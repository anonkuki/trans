<template>
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    ...
    >
      ...
      <!-- 所属角色下拉列表 -->
      <el-form-item label="所属角色" prop="creatorRole">
        <el-select v-model="formData.creatorRole" clearable placeholder="请选择所属角色"
...
const formData = ref({
  ...
  creatorRole: undefined as number | undefined, // 新增：所属角色ID字段
  remark: ''
})
...
// 所属角色相关
const roleList = ref<RoleApi.RoleVO[]>([]) // 新增：角色列表数据
const roleListLoading = ref(false) // 新增：角色列表加载状态
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      ...
      formData.value = {
        ...roleData,
        creatorRole: roleData.creatorRole
      }
      // 确保回显时显示的是角色名称而不是ID
      if (roleData.creatorRole && roleList.value.length > 0) {
        const selectedRole = roleList.value.find(role => role.id === roleData.creatorRole)
        if (selectedRole) {
          formData.value.creatorRole = selectedRole.id
        }
      }
    } finally {
...
const getRoleList = async () => {
  ...
  try {
    // 调用获取当前登录角色的API
    roleList.value = await RoleApi.getLoginRoleList()
  } catch (error) {
...
const resetForm = () => {
  formData.value = {
    ...
    sort: undefined,
    status: CommonStatusEnum.ENABLE,
    creatorRole: undefined, // 新增：重置 creatorRole 字段
    remark: ''
  }
