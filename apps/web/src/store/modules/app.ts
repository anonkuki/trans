...
import { getDefaultLayoutForEnvironment } from '@/utils/environment'
...
export const useAppStore = defineStore('app', {
  state: (): AppState => {
    return {
      ...
      layout: wsCache.get(CACHE_KEY.LAYOUT) || getDefaultLayoutForEnvironment(), // layout布局
      isDark: wsCache.get(CACHE_KEY.IS_DARK) || false, // 是否是暗黑模式
      currentSize: wsCache.get('default') || 'default', // 组件尺寸
      theme: wsCache.get(CACHE_KEY.THEME) || {
        // 主题色
        elColorPrimary: '#ee4f12',
        // 左侧菜单边框颜色
        leftMenuBorderColor: '#eee',
        // 左侧菜单背景颜色
        leftMenuBgColor: '#dee2f4',
        // 左侧菜单浅色背景颜色
        leftMenuBgLightColor: '#dee2f4',
        // 左侧菜单选中背景颜色
        leftMenuBgActiveColor: 'rgba(238, 79, 18, 0.1)',
        // 左侧菜单收起选中背景颜色
        leftMenuCollapseBgActiveColor: 'rgba(238, 79, 18, 0.1)',
        // 左侧菜单字体颜色
  ...
  actions: {
    ...
    // 初始化布局，根据环境设置默认布局
    initLayout() {
      // 只有在缓存中没有设置布局时才根据环境设置默认布局
      if (!wsCache.get(CACHE_KEY.LAYOUT)) {
        const defaultLayout = getDefaultLayoutForEnvironment()
        this.layout = defaultLayout as LayoutType
        wsCache.set(CACHE_KEY.LAYOUT, this.layout)
      }
    }
  },
