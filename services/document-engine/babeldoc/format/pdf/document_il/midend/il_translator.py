...
class DocumentTranslateTracker:
    ...
    def to_json(self):
        # list() 快照:translate() 提交阶段主线程仍在 append page/cross_page/cross_column,
        # 实时 flush 线程会并发调用 to_json,直接迭代会抛 "list changed size during iteration"。
        pages = []
        for page in list(self.page):
            paragraphs = self.convert_paragraph(page)
            pages.append({"paragraph": paragraphs})
        cross_page = []
        for page in list(self.cross_page):
            paragraphs = self.convert_paragraph(page)
            cross_page.append({"paragraph": paragraphs})
        cross_column = []
        for page in list(self.cross_column):
            paragraphs = self.convert_paragraph(page)
    ...
    def convert_paragraph(self, page):
        paragraphs = []
        for para in list(page.paragraph):
...
class ILTranslator:
    ...
    class TranslateInput:
        def __init__(
            ...
            # 术语硬约束(glossary_hard)注入的占位符 -> 目标译法 映射。
            # PRE 阶段(_apply_hard_glossary)把命中术语替换成 {gN} 占位符并填此映射;
            # POST 阶段(_restore_hard_glossary)据此把占位符还原成目标译法。
            # 空 dict 表示未开启或本段无命中术语。
            self.glossary_term_tokens: dict[str, str] = {}
    ...
    def pre_translate_paragraph(
        ...
        if not translate_input:
            return None, None
        # 术语硬约束:开启且本段有命中术语时,把术语替换成 {gN} 占位符,
        # 翻译后再还原成目标译法——保证术语译法不依赖模型是否听话。
        # 在 tracker.set_input 之前做,使 tracker 记录真正发给 LLM 的文本。
        if self.translation_config.glossary_hard and self._cached_glossaries:
            self._apply_hard_glossary(translate_input)
        ...
        # 术语硬约束注入占位符后,原文(如"Sinovac Biotech Ltd.")变成"{g0}"(4 字符),
        # 会跌破 min_text_length 被当成"太短"跳过——译文清空。这里对已注入占位符的
        # 源豁免:长度短是 token 化假象,原文是有内容的术语,不应跳过(否则漏译)。
        has_term_tokens = bool(translate_input.glossary_term_tokens)
        if len(text) < self.translation_config.min_text_length and not has_term_tokens:
            logger.debug(
    ...
    def post_translate_paragraph(
        ...
        # 术语硬约束:把译文里的 {gN} 占位符还原成目标译法。
        # 必须在 tracker.set_output / parse_translate_output 之前(纯文本层)还原:
        # 否则 tracker 记录的是还原前的 {gN}(对照表 CSV 会显示占位符),
        # 且占位符会被当成普通文本塞进 PdfParagraphComposition。
        term_tokens = getattr(translate_input, "glossary_term_tokens", {}) or {}
        if term_tokens:
            translated_text = self._restore_hard_glossary(
                translated_text, translate_input, term_tokens
            )
    ...
    # -------------------------------------------------- 术语硬约束(glossary_hard)
    # 占位符形状 {gN}:前缀 g 与公式占位符 {vN} 区分(公式正则要求 v,不会误匹配);
    # 还原在 parse_translate_output 之前(纯文本层),不进占位符剥离逻辑。

    _GLOSSARY_TERM_TOKEN_RE = re.compile(r"\{g(\d+)\}")

    def _apply_hard_glossary(self, translate_input) -> None:
        """PRE:把命中术语替换成 {gN} 占位符,填 glossary_term_tokens 映射。

        修改 translate_input.unicode 与 .glossary_term_tokens。
        """
        text = translate_input.unicode or ""
        if not text:
            return
        # 收集所有 cached glossary 在原文上的命中 span(hyperscan 候选过滤 + finditer 定位)
        spans: list[tuple[int, int, str]] = []  # (start, end, target)
        for glossary in self._cached_glossaries:
            for _source, target, start, end in glossary.get_active_spans_for_text(text):
                spans.append((start, end, target))
        if not spans:
            return
        # 非重叠选择:按 (start 升序, 长度降序) 排序后贪心选 start >= last_end 的。
        # 长术语优先(如 "Sinovac Biotech Ltd." 优先于内部 "Biotech")。
        ...
        if not selected:
            return
        # 从左到右编号,建 token -> target 映射;从右到左替换以保留偏移。
        selected.sort(key=lambda x: x[0])
        token_map: dict[str, str] = {}
        spans_with_tokens: list[tuple[int, int, str]] = []
        for idx, (start, end, target) in enumerate(selected):
            token = f"{{g{idx}}}"
            token_map[token] = target
            spans_with_tokens.append((start, end, token))
        new_text = text
        for start, end, token in sorted(spans_with_tokens, key=lambda x: x[0], reverse=True):
            new_text = new_text[:start] + token + new_text[end:]
        translate_input.unicode = new_text
        translate_input.glossary_term_tokens = token_map

    def _restore_hard_glossary(
        self,
        translated_text: str,
        translate_input,
        term_tokens: dict[str, str],
    ) -> str:
        """POST:把译文里的 {gN} 占位符还原成目标译法。

        含纯占位符源边界情况(对齐 RetainPDF metadata.py 修复):整段源文本就是
        一个/几个术语占位符(去掉 {gN} 后无正文)且 LLM 返回空/丢了占位符时,
        直接按源里占位符顺序拼目标译法,避免术语反而漏译。
        """
        if not translated_text:
            translated_text = ""
        restored = translated_text
        for token, target in term_tokens.items():
            restored = restored.replace(token, target)
        # 纯占位符源边界情况
        source = getattr(translate_input, "unicode", "") or ""
        stripped_source = self._GLOSSARY_TERM_TOKEN_RE.sub("", source).strip()
        if not stripped_source:
            # 源去掉占位符后无正文 → 整段是术语占位符。
            # 若还原后仍为空(或不含任何目标译法),按源里占位符顺序重建。
            if not restored.strip():
                ordered_tokens = self._GLOSSARY_TERM_TOKEN_RE.findall(source)
                # findall 返回的是数字组,需还原成 {gN}
                rebuilt = "".join(
                    term_tokens.get(f"{{g{n}}}", "") for n in ordered_tokens
                )
                if rebuilt.strip():
                    restored = rebuilt
        return restored
    ...
    def _build_glossary_block(self, text: str) -> str:
        ...
        # 术语硬约束已把命中术语替换成 {gN} 占位符时,提示模型原样保留占位符。
        hard_glossary_note = ""
        if getattr(self.translation_config, "glossary_hard", False) and self._GLOSSARY_TERM_TOKEN_RE.search(text):
            hard_glossary_note = (
                "## Glossary (hard)\n\n"
                "Some terms have been replaced with `{gN}` placeholders (e.g. `{g0}`). "
                "Keep these placeholders exactly as-is in the output: do NOT translate, "
                "rewrite, reorder, or delete them.\n\n"
            )
        ...
        if not glossary_entries_per_glossary:
            # 硬约束占位符说明仍要输出(即便软引导表为空)
            return hard_glossary_note
    ...
    def translate_paragraph(
        ...
        with PbarContext(pbar):
            try:
                ...
                # 空译文(LLM 返回空 content)不能 post_translate:否则 `paragraph.unicode = ""`
                # + 空 composition,段落被清空,渲染成空白。保留原文(fallback 前 ILTranslatorLLMOnly
                # 已把 composition 恢复成原文),宁可未翻译也不渲染空洞。
                if not (translated_text or "").strip():
                    logger.warning(
                        f"empty translation for paragraph {paragraph.debug_id}, keeping original"
                    )
                    return
