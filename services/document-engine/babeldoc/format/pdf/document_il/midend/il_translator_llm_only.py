...
class ILTranslatorLLMOnly:
    ...
    def translate(self, docs: Document) -> None:
        ...
        # 实时 flush:v3 的 tracking 原本只在全部翻完后写一次,运行中 /bilingual 拉到的是空。
        # 这里起一个后台线程周期落盘已翻完的段落,让实时对照表接口能增量读到进度。
        stop_event = threading.Event()
        flush_thread = threading.Thread(
            target=self._flush_tracking_loop,
            args=(tracker, path, stop_event),
            daemon=True,
        )
        flush_thread.start()
        ...
        if (
            ...
            self._flush_tracking_json(tracker, path)
        logger.info(
            f"Translation completed. Total: {self.total_count}, Successful: {self.ok_count}, Fallback: {self.fallback_count}"
        )

    def _flush_tracking_json(
        self, tracker: DocumentTranslateTracker, path: Path
    ) -> None:
        """原子写 translate_tracking.json(tmp 改名,避免读者读到半截文件)。"""
        try:
            tmp = Path(str(path) + ".tmp")
            tmp.write_text(tracker.to_json(), encoding="utf-8")
            tmp.replace(path)
        except Exception:  # noqa: BLE001
            # 提交阶段 to_json 可能撞上列表变更,跳过本次,下个周期再试。
            logger.debug("flush translate_tracking.json skipped", exc_info=True)

    def _flush_tracking_loop(
        self,
        tracker: DocumentTranslateTracker,
        path: Path,
        stop_event: threading.Event,
    ) -> None:
        """后台周期 flush translate_tracking.json,直到 stop_event 置位。"""
    ...
    def translate_paragraph(
        ...
        # 失败兜底:翻译前快照原始 composition+unicode。LLM-only 预处理会改写
        # composition(占位符/样式替换),若翻译失败又不恢复,渲染阶段拿不到原文
        # 字符 → 该段渲染成空白。这里快照,item 失败/整批失败时恢复原文;
        # fallback 简单翻译成功会覆盖,失败则保留原文(不空白)。
        snapshots: dict[int, tuple[object, str]] = {}
        for _p in batch_paragraph.paragraphs:
            snapshots[id(_p)] = (
                copy.deepcopy(_p.pdf_paragraph_composition),
                _p.unicode or "",
            )
        try:
            inputs = []
            ...
            try:
                ...
                # LLM 间歇性返回空/非 JSON 响应(日志 "Expecting value: line 1 column 1"),
                # 不同模型/任务随机命中不同段。直接 fallback 是同一模型再试,可能又空。
                # 这里重试一次,仍失败再交给外层 except 走 fallback(快照兜底保留原文)。
                logger.warning(
                    "LLM JSON unparseable, retrying once. raw_head=%r",
                    llm_output[:200],
                )
                llm_output = self.translate_engine.llm_translate(
                    final_input,
            ...
            for id_, output in translation_results.items():
                ...
                try:
                    ...
                    if not (0.3 < output_token_count / input_token_count < 3):
                        # 术语硬约束纯占位符源(整段是术语占位符,如"{g0}"):input
                        # token 极少,比值常异常,但还原后是完整目标译法,不应判
                        # "太长/太短"走 fallback(否则漏译)。豁免此类输入。
                        _term_tokens = getattr(translate_input, "glossary_term_tokens", {}) or {}
                        _pure_term = bool(_term_tokens) and not re.sub(
                            r"\{g\d+\}", "", trimed_input
                        ).strip()
                        if not _pure_term:
                            llm_translate_tracker.set_error_message(
                    ...
                    if should_fallback:
                        ...
                        # 恢复原始 composition:fallback 成功会覆盖,失败则保留原文。
                        _snap = snapshots.get(id(inputs[id_][2]))
                        if _snap is not None:
                            inputs[id_][2].pdf_paragraph_composition = _snap[0]
                            if _snap[1]:
                                inputs[id_][2].unicode = _snap[1]
                        executor.submit(
            ...
            for input_ in inputs:
                ...
                # 整批失败:恢复原始 composition,避免渲染空白(fallback 成功会覆盖)。
                _snap = snapshots.get(id(input_[2]))
                if _snap is not None:
                    input_[2].pdf_paragraph_composition = _snap[0]
                    if _snap[1]:
                        input_[2].unicode = _snap[1]
    ...
    def _build_llm_prompt(
        ...
        # 术语硬约束:已把命中术语替换成 {gN} 占位符时,提示模型原样保留占位符。
        # 硬替换后这些术语不再出现在 batch 文本里,故上方软引导表为空也仍需此说明。
        if getattr(self.translation_config, "glossary_hard", False) and re.search(
            r"\{g\d+\}", batch_text_for_glossary_matching
        ):
            glossary_usage_rules_block = (
                glossary_usage_rules_block
                + "## Glossary (hard)\n"
