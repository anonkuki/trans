...
def _is_translatable_page_item(item: TextItem, *, enable_table_translation: bool = False) -> bool:
    # 显式策略(policy_translate=True)优先放行 —— 例如 header/metadata 已在
    # OCR 归一化阶段标记为应翻译,但它们的 structure_role 不在默认白名单里,
    # 这里尊重显式策略,避免被结构白名单再次过滤掉。
    if getattr(item, "policy_translate", None) is True:
        return True
    if _get_structure_role(item) in PRIMARY_TRANSLATABLE_STRUCTURE_ROLES:
        return True
    # enable_table_translation 时:表格项不靠 structure_role 白名单判定
    # (table 角色不在 PRIMARY_TRANSLATABLE_STRUCTURE_ROLES 里),按 block_kind 放行。
    return bool(
        enable_table_translation
        and str(getattr(item, "block_kind", "") or "").lower() == "table"
    )
...
def should_translate_block(
    ...
    *,
    ...
    # enable_table_translation 时:表格块直接放行翻译。必须放在 explicit_policy 判定
    # 之前 —— PaddleOCR 归一化会给表格块写 policy.translate=False
    # (provider_non_text:table),若先走 explicit_policy 分支会在 return False 处
    # 被短路丢掉,永远到不了下面的非 text 放行逻辑。仍尊重 skip_translation 标签
    # 与算法块语义的显式拒绝。
    if enable_table_translation and _block_kind(block) == "table":
        if "skip_translation" in normalize_tags(block.get("tags", [])):
            return False
...
def extract_text_items(
    ...
    *,
    ...
    return [
        item
        for item in _apply_page_structure(items)
        if _is_translatable_page_item(item, enable_table_translation=enable_table_translation)
    ]
