...
def mark_policy_skip(
    ...
    *,
    ...
    _bid = str(item.get("item_id", "") or "")
    if _bid == "p001-b004":
        print(f"[DEBUG mark_policy_skip] item_id={_bid} label={label} skip_reason={skip_reason}", flush=True)
        print("".join(_tb.format_stack()[-8:]), flush=True)
...
def mark_keep_origin(item: dict, *, reason: str = KEEP_ORIGIN_LABEL) -> None:
    ...
    _bid = str(item.get("item_id", "") or "")
    if _bid == "p001-b004":
        print(f"[DEBUG mark_keep_origin] item_id={_bid} reason={reason}", flush=True)
        print("".join(_tb.format_stack()[-8:]), flush=True)
