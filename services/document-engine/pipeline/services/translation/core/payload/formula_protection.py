...
def _collect_term_spans(
    text: str,
    glossary_entries: list[GlossaryEntry] | None,
    precomputed_spans: list[tuple[GlossaryEntry, int, int]] | None = None,
) -> list[_Span]:
    selected: list[_Span] = [
    ...
    if precomputed_spans is not None:
        # 硬替换开关开启:复用 hyperscan 已算出的命中位置,不逐条 finditer。
        # 开关开启时所有命中术语都走硬替换(不限 canonical/preserve),restore_text
        # 按 level 决定:preserve 用原文,其余用 target。
        for entry, start, end in precomputed_spans:
...
def protect_glossary_terms(
    ...
    *,
    ...
    if not normalized and precomputed_spans is None:
        return text, list(existing_map or [])
    term_spans = _collect_term_spans(text, normalized, precomputed_spans=precomputed_spans)
    if not term_spans:
        return text, list(existing_map or [])
